...
# ================================
# trainEmbedding.py  (self-contained)
# ================================
...
# -------------------------------
# Helpers (your versions + one correction)
# -------------------------------
...
def _entropy_norm(h, bins):
    ...
    p = h / s
    p = p[p > 0]
    H = float(-(p * np.log(p)).sum())  # natural log
...
def train_contrastive(lines, epochs=10, lr=2e-3, temp=0.07):
    ds = SymDataset(lines)
    dl = DataLoader(ds, batch_size=128, shuffle=True)  # num_workers=0 on Windows by default
    ...
    for ep in range(1, epochs+1):
        ...
        print(f"ep{ep}: train {total/len(ds):.4f}")
    model.eval()
    return model.cpu()
...
if __name__ == "__main__":
    ...
    model = train_contrastive(lines, epochs=10)

    dummy = torch.zeros(1, 256, dtype=torch.float32)
    torch.onnx.export(
        model,
        dummy,
        "aeolus_embedder.onnx",
        input_names=["input"],
